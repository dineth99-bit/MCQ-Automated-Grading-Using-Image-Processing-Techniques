...
########################################################################
webCamFeed = True
pathImage = "Inkedmcq_test_LI.jpg"
...
while True:
    ...
    try:
        ...
        gradePoints = utlis.getCornerPoints(rectCon[1]) # GET CORNER POINTS OF THE Third BIGGEST RECTANGLE
        ...
        if biggestPoints.size != 0 and gradePoints.size != 0:
            ...
            #cv2.imshow("show",imgGradeDisplay)
            ...
            cv2.putText(imgRawGrade,str(int(score))+"%",(50,100)
                        ,cv2.FONT_HERSHEY_TRIPLEX,3,(0,255,255),6) #ADD THE GRADE TO NEW IMAGE
            ...
            cv2.imshow("Grade After Transformation", imgInvGradeDisplay)
            cv2.imshow("wrap",imgInvWarp)
            cv2.imshow("finaltest",imgFinal)

            # SHOW ANSWERS AND GRADE ON FINAL IMAGE
            imgFinal = cv2.addWeighted(imgFinal, 1, imgInvWarp, 1,0)
            #imgFinal = cv2.convertScaleAbs(imgFinal, alpha=1.0,beta=100)  # Increase alpha for contrast, beta for brightness
            imgFinal = cv2.addWeighted(imgFinal, 0.8, imgInvGradeDisplay, 1,10)
    ...
    if cv2.waitKey(1) & 0xFF == ord('s'):
        ...
        cv2.putText(stackedImage, "Paper Saved", (int(stackedImage.shape[1] / 2) - 200, int(stackedImage.shape[0] / 2)),
